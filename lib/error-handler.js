				...
				switch (error.type) {
					case 'INVALID_HEADER':
						return {
							...
							details: `Response header '${error.name}' ('${res.get(error.name)}') ${error.error.message}`,
							source: {
					...
					case 'MISSING_HEADER':
						return {
							...
							details: `Required response header '${error.name}' was missing from the original response.`,
							source: {
								parameter: error.name
							}
						};
					case 'INVALID_BODY':
						return {
							...
							details: `${error.error.property} ${error.error.message}`,
							source: {}
						};
					case 'INVALID_STATUS':
						return {
